...
class IndexedDBService {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(STORE_NAME)) {
          ...
          store = (event.currentTarget as any).transaction.objectStore(
            STORE_NAME
          );
        }
  ...
  public buildIndexes(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      console.log(`Index building took ${end_time.toFixed(2)} ms`);
    });
  }
  ...
  private calculateFuzzyScore(row: Instrument, searchTerm: string): number {
    ...
    for (const candidateWord of candidates) {
      ...
      if (distance <= maxDistance) {
        ...
        for (
          let i = 0;
          i < Math.min(lowerSearchTerm.length, candidateWord.length);
          i++
        ) {
  ...
  public async searchData(
    ...
    for (const token of searchTokens) {
      ...
      if (isFirstToken) {
        ...
        matchingIds = new Set(
          [...matchingIds].filter((id) => idsForToken.has(id))
        );
      }
