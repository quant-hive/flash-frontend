...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative group flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
    <div className="absolute left-2 flex items-center justify-center peer h-4 w-4 shrink-0 focus-visible:outline-1 outline-offset-4 disabled:cursor-not-allowed disabled:opacity-50 rounded-none border border-[#2D2D2D] bg-[#1F1F1F] group-data-[state=checked]:border-[#C3FFAA] group-data-[state=checked]:bg-[#1F1F1F]">
      <SelectPrimitive.ItemIndicator className="flex items-center justify-center bg-green_table_filter_select_foreground size-2"></SelectPrimitive.ItemIndicator>
    </div>
