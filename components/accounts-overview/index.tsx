...
export function AccountsOverview({}: AccountsOverviewProps = {}) {
  const [accounts, setAccounts] = useState<Account[]>(initialAccounts);
  const [isAddMoneyModalOpen, setIsAddMoneyModalOpen] =
    useState<boolean>(false);
  const [isSendMoneyModalOpen, setIsSendMoneyModalOpen] =
    useState<boolean>(false);
  const [isRequestMoneyModalOpen, setIsRequestMoneyModalOpen] =
    useState<boolean>(false);
  ...
  const handleAddMoney = (amount: number): void => {
    setAccounts(
  ...
  const handleSendMoney = (amount: number, fromAccount: string): void => {
    setAccounts(
      accounts.map((account) =>
